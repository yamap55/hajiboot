...
public class CustomerRestControllerTest2 {
	@Autowired
	...
	public void testGetCustomers() throws Exception {
		when().get("/api/customers").then().statusCode(HttpStatus.OK.value()).body("numberOfElements", is(2))
	...
	public void testPostCustomers() throws Exception {
		...
		given().body(customer3).contentType(ContentType.JSON).and().when().post("/api/customers").then()
